...
const int numPoints = 4; // number of points on the circle
...
void loop() {
  ...
  //if (count<6){
    for(int ix = 0; ix < 4; ix++) { //use numPoints instead of 8 later on

      srituhobby.setPWM(servo1, 0, theta_1[ix]);
      srituhobby.setPWM(servo2, 0, theta_2[ix]);
      srituhobby.setPWM(servo3, 0, theta_3[ix]);
...
//  count = count + 1;
//  Serial.print("count of cycle is:");
