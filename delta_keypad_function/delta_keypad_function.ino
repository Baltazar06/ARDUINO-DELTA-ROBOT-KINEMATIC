...
void loop() {
  ...
  while (datakeypad == charinverse) { // if we select the A button

    // move servo motors and end effector to desired location
...
//      Serial.println(th_k[2][ix]);
      ...
      // keep datakeypad as A to move it all the time
      datakeypad = 'A';
  ...
  while (datakeypad == charstop) { // if we select the A button
