...
// circle variables
float rc = 40; // radius of the circle
...
void loop() {
  ...
  switch(datakeypad)
    { 
    case 'A': // move robot with numpoints in the same plane
      // if we reach last position reset the iteration
      if (ix==numPoints){
        ix = 0;
      }

    while(ix < numPoints) { // move robot in the desired range
      // read time passed
      unsigned long currentMillis = millis();
      if (currentMillis - previousMillis >= interval) {// move robot after each 500ms
        ...
        //Serial.println(datakeypad);
        // attach servo motors
        // myservodriver.setPWM(servo1, 0, activePosition);
        // myservodriver.setPWM(servo2, 0, activePosition);
        // myservodriver.setPWM(servo3, 0, activePosition);
      ...
      break;
    case 'B': // hold robot for a moment
    
    float th1d = 314, th2d = 314, th3d = 314;
    myservodriver.setPWM(servo1, 0, th1d);
    myservodriver.setPWM(servo2, 0, th2d);
    myservodriver.setPWM(servo3, 0, th3d);
    Serial.println(th1d);

    default:// as a default case we do nothing
